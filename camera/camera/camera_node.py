...
class CameraNode(Node):
    ...
    def __init__(self):
        ...
        self.service_topic = (
            self.get_parameter("topic_service").get_parameter_value().string_value
        )
        self.publisher_topic = (
            self.get_parameter("topic_pub").get_parameter_value().string_value
        )
        self.publisher_topic_bw = (
            self.get_parameter("bw_pub").get_parameter_value().string_value
        )
        ...
        self.cam_pubs = self.create_publisher(
            CompressedImage,
            self.publisher_topic,
        ...
        self.service_activation = self.create_service(
            SetBool,
            self.service_topic,
            self.start_cameras_callback,
            callback_group=self.callback_group,
        )

        self.thread = threading.Thread(
            target=self.camera.publish_feeds, args=(self.devrule,)
        )
        ...
        if request.data:
            ...
            if self.thread is None or not self.thread.is_alive():
                if self.camera_type == "oakd_stereo":
                    ...
                    self.thread = threading.Thread(
                        target=self.camera.publish_feeds,
                        kwargs={"devrule": self.devrule, "stop_event": self.stop_event},
                        daemon=True,
                    )
                self.thread.start()
            ...
            # IMPORTANT: ask camera to close device NOW
    ...
    def calculate_bandwidth(self, current_time, previous_time, compressed_image_len):
        ...
        bw = Float32()
        bw.data = float(compressed_image_len * 8) / (
